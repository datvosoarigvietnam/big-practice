...
export default function SidebarMobile({
  ...
  return (
    <div
      className={`lg:hidden fixed top-0 left-0 h-full w-full bg-[#83848b] bg-opacity-80 transition-transform transform  scrollbar-thin scrollbar-thumb-gray-500 scrollbar-thumb-rounded-full scrollbar-track-gray-300${
        showSidebarMenu ? 'translate-x-0' : '-translate-x-full'
      } duration-300 ease-in-out z-50 `}
      onClick={onClose}
    >
      <div
        className=" lg:hidden w-[240px] h-[100vh] bg-[#152259] transform transition-transform ease-in-out duration-300 "
        onClick={(event) => {
          event.stopPropagation();
        }}
      >
        <div className="pt-[26px] z-50">
          {/* Header */}
